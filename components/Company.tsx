...
export default function Company(props: CompanyProps) {
  ...
  const renderProfessionalOrganizations = () => {
    ...
    return (
      <ul>
        {orgs.map(org => (
          <li key={org}>{profOrgsNameMap[org] || org}</li>
        ))}
      </ul>
    );  
  }
  ...
  return (
    ...
    >
      ...
      {data.contact_info && (
      <section>
        <h2 className="text-lg mt-4 mb-2 font-semibold">Contact Details</h2>
          <div>
            <div className="mb-4">
            ...
            {data.contact_info.email && <p>Email: <a href={`mailto:${data.contact_info.email}`}>{data.contact_info.email}</a></p>}
            {data.contact_info.phone && <p>Phone: <a href={`tel:${data.contact_info.phone}`}>{data.contact_info.phone}</a></p>}
            {data.contact_info.mobile && <p>Mobile: <a href={`tel:${data.contact_info.mobile}`}>{data.contact_info.mobile}</a></p>}
            {data.contact_info.website && <p>Website: <a href={data.contact_info.website} target="_blank" rel="noopener noreferrer">{data.contact_info.website}</a></p>}

            <div className="flex gap-1 items-center mt-4 *:basis-6 *:shrink-0 *:grow-0">
              {(data.contact_info.facebook || data.contact_info.linkedin || data.contact_info.twitter || data.contact_info.youtube) && (
                <p>Social&nbsp;Media:</p>
              )}

              {data.contact_info.facebook && (
                <a href={checkFacebookLinkAndTransform(data.contact_info.facebook)?.toString()} target="_blank" rel="noopener noreferrer">
                  <svg className="w-full h-auto" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="100" height="100" viewBox="0 0 24 24">
              ...
              {data.contact_info.linkedin && (
                <a href={checkLinkedInLinkAndTransform(data.contact_info.linkedin)?.toString()} target="_blank" rel="noopener noreferrer">
                  <svg className="w-full h-auto" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="100" height="100" viewBox="0 0 24 24">
              ...
              {data.contact_info.twitter && (
                <a href={checkTwitterLinkAndTransform(data.contact_info.twitter)?.toString()} target="_blank" rel="noopener noreferrer">
                  <svg className="w-full h-auto" viewBox="0 -1 27 27" width="100" height="100" aria-hidden="true"><g><path fill="currentColor" d="M21.742 21.75l-7.563-11.179 7.056-8.321h-2.456l-5.691 6.714-4.54-6.714H2.359l7.29 10.776L2.25 21.75h2.456l6.035-7.118 4.818 7.118h6.191-.008zM7.739 3.818L18.81 20.182h-2.447L5.29 3.818h2.447z"></path></g></svg>
                </a>
              )}  
              {data.contact_info.youtube && (
                <a href={checkYouTubeLinkAndTransform(data.contact_info.youtube)?.toString()} target="_blank" rel="noopener noreferrer">
                  <svg className="w-full h-auto" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="100" height="100" viewBox="0 0 24 24">
      ...
      <section>
          <div>
            {Object.entries(data.areas).map(([category, areas]) => (
              <div key={category}>
                <h2 className="text-lg font-semibold mt-4 mb-4">{category}</h2>
                <ul>
      ...
      {data.established && data.type && (
      <section>
        <h2 className="text-lg mt-4 mb-2 font-semibold">Key Information:</h2>
        <p>{data.name} is a {data.type} established in {data.established}.</p>
      </section>
      )}

      {data.employees && data.employees.length > 0 && (
      <section>
        <h2 className="text-lg mt-4 mb-2 font-semibold">Employees who are AQR members:</h2>
        <ul>
      ...
      {data.proforgs && (
      <section>
        <h2 className="text-lg mt-4 mb-2 font-semibold">
      ...
      <section>
        ...
        <ul>
          {data.accred.split(',').map((accreditation, index) => (
            <li key={index}>{accreditation.trim()}</li>
          ))}
        </ul>
      </section>
      )}
      ...
      <section>
        <h2 className="text-lg mt-4 mb-2 font-semibold">Training Opportunities</h2>
        <p>{data.name} operates a graduate training programme. Please contact {data.name} directly for more information about their training programme and employment opportunities.</p>
      </section>    
      )}

      {data.prsaward && (
      <section>
        <h2 className="text-lg mt-4 mb-2 font-semibold">AQR Awards</h2>
        <div 
          className="mb-4 *:my-4" 
        >
          {renderAwards()}
