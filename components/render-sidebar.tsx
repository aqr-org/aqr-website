        ...
        <div key={item.name+index} className={cn(index === 0 ? "mt-0" : "mt-8")}>
          {item.component === "navigation_cta" ? (
            ...
            <Link 
              ...
              className="text-lg font-semibold text-qlack"
            >
              {item.name}
            </Link>
          ) : (
            <div className="text-lg font-semibold text-qlack">
              {item.name}
            </div>
          )}
          {item.dropdown_menu &&
              ...
              {item.dropdown_menu.map((dropdownItem: NavigationLinkData, dropdownIndex: number) => (
                <li key={dropdownItem.name+dropdownIndex}>
                  <Link 
                    ...
                    className="group text-[1rem] text-qreen-dark flex gap-1 items-start"
                  >
